...
If `package.json` or `package-lock.json` is staged, `.husky/pre-commit` runs `scripts/snyk-precommit.sh`, which executes `snyk test --severity-threshold=high` **only when** the real `snyk` CLI runs (`snyk version` succeeds — IDE-only shims are skipped) **and** `snyk auth` / `SNYK_TOKEN` is configured. Otherwise it prints a skip message and continues.
...
- **Secrets in history:** [Gitleaks](https://github.com/gitleaks/gitleaks) in CI (`security-> git -c user.useConfigOnly=true commit --quiet --allow-empty-message --file -`
...
`ID: urn:snyk:interaction:5aeac9da-00dd-4fd6-9c34-14179fda3c6d` 
`husky - pre-commit script failed (code 2)`
- `secrets.yml`) on PRs and `main` pushes. GitHub [secret scanning](https://docs.github.com/code-security/secret-scanning/about-secret-scanning) applies when enabled for the org/repo.
