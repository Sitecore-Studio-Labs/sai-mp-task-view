...
**Field-level annotations** (`fields.<name>` block):

- **`from`** _(required)_ — Dot-path into the raw API object (e.g. `dates.due`). Paths starting with `fields.` are nested inside a `fields:{}` block in the generated normalizer.
- **`nullable`** — When `true`, appends `?? undefined` so absent optional fields don't produce `null`.
- **`transform`** — `"self-array"` (recursive normalizeTask for subtasks), `"comments-array-wrapper"` (nested comment list), or any platform-specific string. Unknown transforms emit a typed TODO stub with the available resolution maps listed in a comment.
- **`type`** — Explicit TypeScript type for the generated interface field, overriding the heuristic (e.g. `type: '"High" | "Normal" | "Low"'`).
- **`enumValues`** — List of allowed string values. Generates a union type in `types.ts` (e.g. `enumValues: [High, Normal, Low]` → `"High" | "Normal" | "Low"`).

**Entity-level annotations**:

- **`requiresResolution`** — Array of `{ name, mapValueType, importedFrom }` objects. When present, the generated normalizer function receives typed `Map<string, T>` parameters that the adapter must pre-fetch and inject. Used when raw API fields are IDs that require lookup (e.g. Wrike `responsibleIds` → contacts map, `customStatusId` → status map).
...
| `validate-http-adapter` | Exits 1 if any public async method in `<Platform>Adapter.ts` is missing from `<Platform>HttpAdapter.ts` |
...
# Verify the HTTP adapter interface is complete
npx nx run wrike:validate-http-adapter
```
