...
│   │   ├── storeConfig.ts                      # TRELLO_STORE_CONFIG (Postgres table/column names)
│   │   ├── tokenStore.ts                       # createTrelloTokenStore()
...
Exchange the `code` query parameter for access + refresh tokens. Store them via `PostgresTokenStore` and set the session cookie. Redirect back to the app.
...
For **OAuth platforms** (`hasOAuth: true`) this file is fully generated — it reads the session cookie and resolves the userId via `PostgresTokenStore.lookupSession`. No changes needed.

For **non-OAuth platforms** (`hasOAuth: false`, e.g. API key) a `// TODO:` stub is generated instead. Implement `getTrelloUserIdFromSession` to return the userId (the key used to look up the user's stored credentials in Azure PostgreSQL) or `null` if the request is unauthenticated. Common patterns: decode a JWT from an Authorization header, look up an API key owner, or read from a session cookie.
...
- Mock every function imported from `@/services/<platform>Service` — do not let tests hit the database or platform APIs.
