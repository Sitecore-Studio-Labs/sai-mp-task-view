...
2. **Edit if needed** — edit `CHANGELOG.md` directly on GitHub and commit to the branch. The publish workflow re-derives the GitHub Release body from `CHANGELOG.md` at publish time, so this is the only file you need to edit.
...
- Create the GitHub Release using `.release-notes.md` as the release body
...
If you edit `CHANGELOG.md` on the draft branch, those edits will automatically
appear in the GitHub Release body — the publish workflow re-derives
`.release-notes.md` from `CHANGELOG.md` at publish time.
