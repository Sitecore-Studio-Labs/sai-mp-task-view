...
function CircularProgress({
  ...
  const CircularProgressElement = (
    <div className="text-center">
      ...
      {message && <p className="text-subtle-text mt-6 text-sm">{message}</p>}
    </div>
  );

  if (fullscreen) {
    return (
      <div className="bg-body-bg/90 fixed inset-0 z-50 flex items-center justify-center">
  ...
  if (withOverlay) {
    return (
      <div className="bg-body-bg/90 flex h-full w-full items-center justify-center">
        {CircularProgressElement}
      </div>
    );
  }
