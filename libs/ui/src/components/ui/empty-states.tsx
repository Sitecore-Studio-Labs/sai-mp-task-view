...
interface EmptyStatesProps
  extends React.ComponentProps<"div">, VariantProps<typeof emptyStatesVariants> {
  title?: string;
  description?: string;
  imageSrc?: string;
  imageAlt?: string;
  ...
  return (
    ...
    >
      <div className="flex max-w-sm flex-col items-center gap-4 text-center">
        <img src={finalImageSrc} alt={finalImageAlt} className="h-16 w-16" />
        <div className="flex flex-col items-center gap-1.5">
          <h2 className="text-lg font-semibold">{finalTitle}</h2>
          {finalDescription && <p className="text-muted-foreground text-sm">{finalDescription}</p>}
        </div>
        {finalActions}
      </div>
    </div>
