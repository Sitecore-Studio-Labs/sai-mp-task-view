...
interface ErrorStatesProps
  extends React.ComponentProps<"div">, VariantProps<typeof errorStatesVariants> {
  title?: string;
...
function ErrorStates({
  ...
  const finalErrorCode =
    errorCode || ("defaultErrorCode" in config ? config.defaultErrorCode : undefined);
  ...
  return (
    ...
    >
      <div className="flex max-w-sm flex-col items-center gap-6 text-center">
        <img src={finalImageSrc} alt={finalImageAlt} className="h-32 w-32" />
        <div className="flex flex-col items-center gap-1.5">
          <h1 className="text-2xl font-semibold">{finalTitle}</h1>
          {finalErrorCode && <p className="text-muted-foreground text-sm">{finalErrorCode}</p>}
        </div>
