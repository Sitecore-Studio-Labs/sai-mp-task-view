      ...
      <div className="flex shrink-0 items-start">
        ...
        <div className="flex shrink-0 flex-col items-center">
          <div className={cn(stepIconVariants({ status, size }))}>
            {status === "completed" ? <CheckIcon className="size-4" /> : <span>{index + 1}</span>}
          </div>
        </div>
        ...
        <div className="ml-3 flex min-w-0 flex-col">
          ...
          {step.description && (
            <div className={cn(stepDescriptionVariants({ status }))}>{step.description}</div>
          )}
        </div>
      </div>
...
export function Stepper({
        ...
        {stepsWithStatus.map((step, index) => (
          <div key={index} className="flex flex-1 gap-4">
            ...
            <div className="flex flex-1 flex-col pb-4">
              <div className={cn(stepLabelVariants({ status: step.status || "pending" }))}>
                {step.label}
              </div>
              {step.description && (
                <div
  ...
  return (
    <div
      className={cn("bg-muted/30 flex w-full items-center gap-4 rounded-lg p-6", className)}
      {...props}
    >
      {stepsWithStatus.map((step, index) => (
        <StepperStepItem
