...
function CommandInput({
  ...
  return (
    <div data-slot="command-input-wrapper" className="relative my-2 px-3">
      ...
      <SvgIcon
        path={mdiMagnify}
        className="absolute top-1/2 left-6 size-6 shrink-0 -translate-y-1/2 opacity-50"
      />
      {value && (
        <Button
          onClick={handleClear}
...
function CommandItem({ className, ...props }: React.ComponentProps<typeof CommandPrimitive.Item>) {
  return (
    <CommandPrimitive.Item
      ...
      className={cn(
        "data-[selected=true]:text-foreground [&_svg:not([class*='text-'])]:text-muted-foreground data-[selected=true]:bg-neutral-bg relative flex cursor-default items-center gap-2 rounded-sm px-2 py-1.5 text-sm outline-hidden select-none data-[disabled=true]:pointer-events-none data-[disabled=true]:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className,
      )}
      {...props}
    />
