...
async function publishDraft(draftId: string, projectId: string): Promise<PublishResult> {
  const res = await fetch(`/api/workbreakdown/${encodeURIComponent(draftId)}/publish`, {
    method: "POST",
...
export function usePublishWorkBreakdown(
  ...
  return useMutation({
    mutationFn: ({ projectId }: { projectId: string }) => publishDraft(draftId!, projectId),
    onSuccess: (result) => {
      if (draftId) {
